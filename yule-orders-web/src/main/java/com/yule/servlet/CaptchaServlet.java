...
public class CaptchaServlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        try {
            ...
            session.removeAttribute(CaptchaSessionConst.ORDERS_CAPTCHA_KEY);
            session.setAttribute(CaptchaSessionConst.ORDERS_CAPTCHA_KEY, EncryptUtil.encryptToMD5(randomString.toLowerCase()));
        	//将内存中的图片通过流动形式输出到客户端
