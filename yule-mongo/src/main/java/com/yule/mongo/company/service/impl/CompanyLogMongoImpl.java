...
public class CompanyLogMongoImpl implements ICompanyLogMongo{
	...
	public Page<CompanyLog> findCompanyLogPage(CompanyLogQuery companyLogQuery,int pageSize,int pageNo) throws YuleException{
		...
		List<Integer> status = companyLogQuery.getType();
	...
	public boolean insertCompanyLog(CompanyLog companyLog)  throws YuleException {
		boolean flag = false;
		try{
			if(null==companyLog.getCreate_time()){
				companyLog.setCreate_time(DateUtil.getCurrentDate());
			}
			this.mongoTemplate.save(companyLog);
			flag = true;
		}catch(Exception e){
