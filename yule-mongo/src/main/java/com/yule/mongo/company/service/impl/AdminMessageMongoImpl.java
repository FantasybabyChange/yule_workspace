...
public class AdminMessageMongoImpl implements IAdminMessageMongo{
	...
	public Page<AdminMessage> findAdminMessagePage(AdminMessageQuery adminMessageQuery,int pageSize,int pageNo) throws YuleException{
		...
		if(!StringUtils.isEmpty(adminMessageQuery.getAdmin_user_id())){
			query.addCriteria(new Criteria("admin_user_id").is(adminMessageQuery.getAdmin_user_id()));
		}
		if(!StringUtils.isEmpty(adminMessageQuery.getContent())){
			query.addCriteria(new Criteria("content").regex(adminMessageQuery.getContent()));
		}
	...
	public long findAdminMessageIsReadCount(String receiveId,String isRead) throws YuleException {
		Query query = new Query();
	...
	public boolean batchInsertAdminMessage(List<AdminMessage> adminMessages)
	...
	public int findAdminMessageCount(AdminMessageQuery adminMessageQuery) throws YuleException {
		...
		query.addCriteria(new Criteria("is_delete").is(DeleteConst.IS_DELETE_TRUE));
		query.addCriteria(new Criteria("receive_id").is(adminMessageQuery.getReceive_id()));
		return ConvertUtil.longToInteger(this.mongoTemplate.count(query, AdminMessage.class));
	}
