...
;(function($, undefined) {
	...
	$(document).on('click.' + pluginName, '[data-trigger="ajax"]', function() {
		...
		if (typeof data['target'] != 'undefined') {
			...
			if($this.parent().find($target).parent().is(':visible')){
				$this.parent().find($target).parent().hide()
				return false;
			}
			$target.load($this.attr('href'), function() {
				$('.ajax-content').hide()
				$this.parent().find($target).parent().show()
				if (spinner) {
					spinner.stop();
				}
			});
	...
	$('[data-trigger="ajax"],.ajax-content').mouseup(function(event) {
		return false;
	});
