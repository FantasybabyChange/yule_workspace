...
import com.yule.admin.dao.ICompanyPhoneDao;
...
public class CompanyPhoneServiceImpl implements ICompanyPhoneService {
	...
	public List<CompanyPhone> findCompanyPhoneListByCompanyId(String companyId) throws YuleException {
		...
		return this.companyPhoneDao.findCompanyPhoneList(params);
	}
	...
	public boolean batchInsertAndUpdateCompanyPhone(InsertCompanyPhoneParam insertCompanyPhoneParam) throws YuleException {
		...
		try {
			...
			if(null != lists && lists.size()>0){
				List<CompanyPhone> insertCompanyPhones = new ArrayList<CompanyPhone>();
				List<CompanyPhone> updateCompanyPhones = new ArrayList<CompanyPhone>();
				String companyId = insertCompanyPhoneParam.getCompany_id();
				...
				for (String id : lists) {
					...
					companyPhone.setCompany_id(companyId);
					companyPhone.setName(names.get(i));
					companyPhone.setPhone(phones.get(i));
					if(StringUtils.isEmpty(id)){
						companyPhone.setId(IDUtil.getID());
						insertCompanyPhones.add(companyPhone);
					}else{
				...
				names = null;
				phones = null;
				if(insertCompanyPhones.size() >0){
					this.companyPhoneDao.batchInsertCompanyPhone(insertCompanyPhones);
					insertCompanyPhones.clear();
				}
				insertCompanyPhones=null;
				if(updateCompanyPhones.size() >0){
