...
@Controller
@RequestMapping("/salesmanImage")
public class SalesmanImageUploadAction extends BaseAction{
	

	private static final String upload_dir ="/salesman/";
	/**
	 * 后台上传文件
	 * @return
	 * @throws Exception
	...
	public String upload(@RequestParam(value = "files[]", required = false) MultipartFile[] fileUploads,@RequestParam(value = "params", required = false)String params) throws Exception {
		...
		String cookieValue = CookieUtil.getCookieValue(request.getCookies(), CookieConst.SALESMAN_COOKIE_NAME);
